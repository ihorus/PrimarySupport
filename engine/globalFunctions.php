...
function gatekeeperCheck($pageAccess = 99) {
	...
	if (is_null($_SESSION[SITE_UNIQUE_KEY]['cUser']['uid'])) {
...
function isAdmin($userUID = NULL) {
	if ($userUID == NULL) {
		...
		$userUID = $_SESSION[SITE_UNIQUE_KEY]['cUser']['uid'];
	}
...
function ps_encrypt($stringToEncrypt = NULL) {
	$inputlen = strlen($stringToEncrypt);// Counts number characters in string $input
	$randkey = rand(1, 9); // Gets a random number between 1 and 9
	
	$i = 0;
	
	while ($i < $inputlen) {
		$inputchr[$i] = (ord($stringToEncrypt[$i]) - $randkey);//encrpytion 
...
function ps_decrypt($stringToDecrypt) {
	$input_count = strlen($stringToDecrypt);
	$dec = explode(".", $stringToDecrypt);// splits up the string to any array
	$x = count($dec);
