...
class Response extends DatabaseObject {
	...
	public function create() {
		...
		if (!$this->description == "") {
			...
			if ($database->query($sql)) {
				...
				$sendEmailOnResponse = UserSettings::get_setting($_SESSION[SITE_UNIQUE_KEY]['cUser']['uid'], "email_send_on_support_response");
	...
	public function create_info() {
		...
		$sql .= $database->escape_value($this->spawn) . "', '";
		$sql .= $database->escape_value($this->description) . "', '";
		$sql .= $database->escape_value($_SESSION[SITE_UNIQUE_KEY]['cUser']['uid']) . "')";
	...
	public function  promote_to_job() {
		...
		$description = "-- created from a response to job  " . $originalJob->uid . " by " . $_SESSION[SITE_UNIQUE_KEY]['cUser']['firstname'] . " --<br />";
